...
const { help, snk, about, broadcast, donate, readme, listChannel } = require('./lib/help')
const { topupdm, igshop } = require('./lib/promosi')
...
module.exports = msgHandler = async (client, message) => {
    try {
        ...
        const apiKey = 'ZSarijvbaKNDYNOTfv7R' // apikey you can get it at https://mhankbarbar.herokuapp.com/api
        ...
        const ownerNumber = ["6285645319608@c.us","6285645319608"] // replace with your whatsapp number
	    ...
	    const ttsJp = require('node-gtts')('ja')
            ...
            if (dataText === '') return client.reply(from, 'Salah Anjim', id)
        ...
        case '!ytmp4':
            ...
            try {
                ...
                if (ytv.error) {
                    ...
                    await client.sendFileFromUrl(from, ytv.result, `${ytv.title}.mp4`, 'Nih bund', id).catch(() => client.reply(from, mess.error.Yt4, id))
                }
            } catch (er) {
        ...
        case '!fb':
            ...
            break
        case '!creator':
            client.sendContact(from, '6285645319608@c.us')
            break
        case '!ig':
            ...
            try {
                ...
                await client.sendFileFromUrl(from, resp.result, `igeh${ext}`, 'Nih bund', id)
            } catch {
        ...
        case '!welcome':
            if (!isGroupMsg) return client.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isGroupAdmins) return client.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin group!', id)
            if (args.length === 1) return client.reply(from, 'Pilih *ON* atau *OFF*', id)
            if (args[1].toLowerCase() === 'on') {
                ...
                client.reply(from, 'Pilih ON atau OFF udin!', id)
            }
            break
        case '!nsfwmenu':
            if (!isNsfw) return
        ...
        case '!linkgroup':
            ...
            if (isGroupMsg) {
                ...
                client.sendLinkWithAutoPreview(from, `╔══✪〘 Link Group 〙✪══\n ${inviteLink}`, '\n\n╚═〘 Powered by Yanxcod3 BOT 〙')
            } else {
        ...
        case '!ownergroup':
            ...
            let owner = '╔══✪〘 Owner Group 〙✪══\n'
                ...
                owner += ` @${Owner_}\n`
                owner += '╠\n'
                owner += '╚═〘 Powered by Yanxcod3 BOT 〙'
            await client.sendTextWithMentions(from, owner)
            break
        case '!mentionall':
            if (!isGroupMsg) return client.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isGroupAdmins) return client.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id)
        ...
        case '!kickall':
            ...
            client.reply(from, 'Kick All Member *SUCCESS', id)
            break
        case '!leaveall':
            if (!isOwner) return client.reply(from, 'Perintah ini hanya untuk Owner bot', id)
        ...
        case '!kick':
            ...
            await client.sendText(from, `Perintah Diterima, Mengeluarkan Member *SUCCESS*`)
        ...
        case '!leave':
            if (!isGroupMsg) return client.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isGroupAdmins) return client.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id)
            await client.sendText(from,'Terima kasih telah menggunakan *Yanxcod3 Bot*').then(() => client.leaveGroup(groupId))
            break
        case '!promote':
            if (!isGroupMsg) return client.reply(from, 'Fitur ini hanya bisa di gunakan dalam group', id)
            ...
            await client.sendTextWithMentions(from, `Perintah Diterima, Menambahkan si @${mentionedJidList[0]} Sebagai Admin.`)
            break
        case '!demote':
            if (!isGroupMsg) return client.reply(from, 'Fitur ini hanya bisa di gunakan dalam group', id)
            if (!isGroupAdmins) return client.reply(from, 'Fitur ini hanya bisa di gunakan oleh admin group', id)
            ...
            await client.sendTextWithMentions(from, `Perintah Diterima, Menghapus Jabatan si @${mentionedJidList[0]}.`)
            break
        case '!delete':
            if (!isGroupMsg) return client.reply(from, 'Fitur ini hanya bisa di gunakan dalam group', id)
            if (!isGroupAdmins) return client.reply(from, 'Fitur ini hanya bisa di gunakan oleh admin group', id)
        ...
        case '!joinbot':
            ...
            if (isGroupMsg) return client.reply(from, 'Maaf, Fitur Ini Hanya Bisa Digunakan Dalam Personal Chat!', id)
            const link = args[1]
            const key = args[2]
            const tGr = await client.getAllGroups()
            const minMem = 10 //numbermember
            const isLink = link.match(/(https:\/\/chat.whatsapp.com)/gi)
            if (key !== 'yanxcod3bot') return client.reply(from, 'Key tidak Valid!', id)
            const check = await client.inviteInfo(link)
            if (!isLink) return client.reply(from, 'Ini link? 👊🤬', id)
            if (check.size < minMem) return client.reply(from, 'Bot Gagal Masuk, Harap Lapor Ke Owner Bot', id)
            if (check.status === 200) {
                await client.joinGroupViaLink(link).then(() => client.reply(from, 'Menambahkan Bot : *SUCCESS*'))
            } else {
        ...
        case '!infobot':
            ...
            client.reply(from, `╔══════✪〘 Info Bot〙✪══════\n╠ \n╠ Loaded Messages : *${loadedMsg} messages*\n╠ Total Group Chats : *${groups.length} chats*\n╠ Total Personal Chats : *${chatIds.length - groups.length} chats*\n╠ Total Semua Chats : *${chatIds.length} chats*\n╠\n╠ 🖥️ Info Server 🖥️\n╠ *User :* Yanxcod3 WhatsApp-Bot\n╠ *Penggunaan RAM :* ${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB / ${Math.round(require('os').totalmem / 1024 / 1024)}MB\n╠ *CPU :* ${os.cpus()[0].model}\n╠\n╚══〘 Powered by Yanxcod3 BOT 〙══`, id)   
            break
        case '!topupdm':
            if (isGroupMsg) return client.reply(from, 'Maaf, Fitur Ini Hanya Bisa Digunakan Dalam Personal Chat!', id)
            client.reply(from, topupdm, id)
            break
        case '!igshop':
            if (isGroupMsg) return client.reply(from, 'Maaf, Fitur Ini Hanya Bisa Digunakan Dalam Personal Chat!', id)
            client.reply(from, igshop, id)
            break
        case '!menu':
       ...
       case '!about':
        ...
        case '#help':
        case '#menu':
            client.reply(from, 'Mungkin yang anda maksud *!menu*', id)
            break        
        case 'p':
        case 'halo':
        case 'kak':
        ...
        case 'hai':
        case 'hay':
            if (isGroupMsg) return client.reply()
            client.reply(from, `Halo Kak *@${pushname}*,\nUntuk Melihat Fitur Ketik *!menu*`, id)
            break
        case 'assalamualaikum':
        case 'assalamualaikum wabarokatuh':
        case 'assalamualaikum wr wb':
        case 'samlekom':
            if (isGroupMsg) return client.reply(from, `Walaikumsallam Kak *@${pushname}*`, id)
            client.reply(from, `Walaikumsallam Kak *${pushname}*,\nKetik *!menu* Untuk Melihat Fitur`, id)
            break    
        case 'makasih':
        case 'thanks':
        case 'tq':
        ...
        case 'thx':
        case 'thankss':
        if (isGroupMsg) return client.reply(from, `Sama Sama Kak *@${pushname}*`, id)
            client.reply(from, `Sama Sama Kak ${pushname}`, id)
         }
    } catch (err) {
